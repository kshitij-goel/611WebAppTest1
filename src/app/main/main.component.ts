...
export class MainComponent implements OnInit {
  ...
  constructor(pubnub: PubNubAngular) {
    pubnub.init({publishKey: this.pubKey, subscribeKey: this.subKey});
    pubnub.grant({
        channels: [this.publishChannel, this.subscribeChannel],
        authKeys: [this.pubKey, this.subKey],
        ttl: 5,
        read: true,
        write: true
      },
  ...
  ngOnInit() {
    ...
    this.yellowDis = true;
    this.greenDis = true;
    this.transmitObject.deviceType = 'webapp';
    // this.transmitObject.setDeviceType('webapp');
  ...
  pubnubPublish(transmitObject: TransmitObject) {
    this.pubnub_global.publish(
      {
        message: JSON.stringify(transmitObject),
  ...
  ClientTask(transmitObject: TransmitObject) {
  ...
  setStatus(red: string, yellow: string, green: string) {
  ...
  setDis(over: string, red: string, yellow: string, green: string) {
